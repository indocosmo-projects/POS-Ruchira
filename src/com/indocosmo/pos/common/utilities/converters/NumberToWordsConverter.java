	...
	public String convertToWords(long n) {
		...
		while (mNumberToConvert.length() > 0) {
			if (pos == 1) // TENS AND UNIT POSITION
			{
				...
				{
					String C = mNumberToConvert.substring(mNumberToConvert.length() - 2,
							mNumberToConvert.length());
			...
			{
				String C = mNumberToConvert.substring(mNumberToConvert.length() - 1,
						mNumberToConvert.length());
			...
			} else if (pos > 2) // REMAINING NUMBERS PAIRED BY TWO
			{
				...
				{
					if (!hun && converted.length() > 0)
						converted = digits(mNumberToConvert) + maxs[pos] + " and"
								+ converted;
					else {
