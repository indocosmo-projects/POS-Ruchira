 ...
 *
 */
public class PosOrderItemDetTaxExclusiveTablePanel extends JPanel {
	...
	private static final int LIST_TABLE_COL_ITEM_NET_INDEX=4;
	private static final int LIST_TABLE_COL_ITEM_TAX_INDEX=5;
	private static final int LIST_TABLE_COL_PRICE_INDEX=6;
	...
	private static final int LIST_TABLE_COL_ITEM_DISC_WIDTH=70;
	private static final int LIST_TABLE_COL_ITEM_TAX_WIDTH=70;
	private static final int LIST_TABLE_COL_PRICE_WIDTH=105;
	private static final int LIST_TABLE_COL_ITEM_NET_WIDTH=70;
	 ...
	 * 
	 */
	public PosOrderItemDetTaxExclusiveTablePanel(JPanel parent, int width, int height  ) {
...
//		orderDetTable.setFont(getFont().deriveFont(18f));
		...
		orderDetTable.getColumnModel().getColumn(LIST_TABLE_COL_ITEM_NET_INDEX).setCellRenderer(rightAlign);
	...
	private void setColumnWidth(JTable orderDetTable){
		...
		orderDetTable.getColumnModel().getColumn(LIST_TABLE_COL_ITEM_NET_INDEX).setPreferredWidth(LIST_TABLE_COL_ITEM_NET_WIDTH );
	...
	private String[] getColumnNames(){
		...
		colNames[LIST_TABLE_COL_ITEM_NET_INDEX]="Net";
	...
	private void SetOrderDetailList(){
		...
		for (BeanOrderDetail item: orderDtlList){
			...
			if(!item.isVoid()){
				...
				mOrderDetailTable.setRowHeight(mOrderDetailTable.getRowCount()-1,ROW_HEIGHT );
				
				if(item.isRefunded()){
					model.addRow(getRowValue(item,true));
					mOrderDetailTable.setRowHeight(mOrderDetailTable.getRowCount()-1,ROW_HEIGHT );
				}
			}
		}
...
//		final String styleEndText=orderDtl.isRefunded()?"</span></span></body></html>":"";
		...
		double price, qty,totalTaxAmt,discount;
		double itemNetAmt,itemGrossAmt;
		
		
		
		if (!isRefdunded){
			
			if(!orderDtl.getSaleItem().getTax().getCode().equalsIgnoreCase("NOTAX") && 
					orderDtl.getSaleItem().getTaxCalculationMethod()==TaxCalculationMethod.InclusiveOfTax){
			
				final double taxRate=PosTaxUtil.getTotalTaxRate(orderDtl.getSaleItem().getTax());
				price=PosOrderUtil.getItemFixedPrice(orderDtl)/(1+taxRate/100);
				
			}else
				price=PosSaleItemUtil.getItemFixedPrice(orderDtl.getSaleItem());
			qty=orderDtl.getSaleItem().getQuantity();	
			totalTaxAmt =PosSaleItemUtil.getTotalTaxAmount(orderDtl.getSaleItem(),true);
			discount=PosSaleItemUtil.getTotalDiscountAmount(orderDtl.getSaleItem());
			itemNetAmt=price*qty-discount;
			itemGrossAmt=PosOrderUtil.getGrandTotal(orderDtl);
		}else{
			
			totalTaxAmt=orderDtl.getRefundTax1Amount() + orderDtl.getRefundTax2Amount() +
					orderDtl.getRefundTax3Amount() + orderDtl.getRefundGSTAmount();
			
			price=(orderDtl.getRefundAmount()-totalTaxAmt)/orderDtl.getRefundQuantity();
			qty=orderDtl.getRefundQuantity();
			discount=0;
			itemNetAmt=price*qty;
			itemGrossAmt=orderDtl.getRefundAmount(); 	
		...
		values[LIST_TABLE_COL_ITEM_PRICE_INDEX] =styleStartText + PosCurrencyUtil.format(price)+ styleEndText;
		values[LIST_TABLE_COL_ITEM_QTY_INDEX] =styleStartText + (isRefdunded? "<font size=4>-</font>":"") +  PosUomUtil.format(qty,orderDtl.getSaleItem().getUom())+ styleEndText;
		values[LIST_TABLE_COL_ITEM_DISC_INDEX] =styleStartText + PosCurrencyUtil.format(discount)+ styleEndText;
		values[LIST_TABLE_COL_ITEM_NET_INDEX] =styleStartText + PosCurrencyUtil.format(itemNetAmt)+ styleEndText;
		
		final String exclTaxSymbol=orderDtl.getSaleItem().getTaxCalculationMethod() ==TaxCalculationMethod.ExclusiveOfTax && totalTaxAmt!=0? " + ":"";
		
		final String taxDataPrefix=styleStartText+ (isRefdunded?"":"<html><body>");
		final String taxDataSuffix=(isRefdunded?"":"</body></html>")+ styleEndText;
		values[LIST_TABLE_COL_ITEM_TAX_INDEX] =  taxDataPrefix + "<font size=4>"+ exclTaxSymbol + "</font>" + 
				PosCurrencyUtil.format(totalTaxAmt) +taxDataSuffix;
	 	values[LIST_TABLE_COL_PRICE_INDEX ]=styleStartText +(isRefdunded? "<font size=4>-</font>":"") +  PosCurrencyUtil.format(itemGrossAmt)+ styleEndText;
